...
show_help() {
  ...
  echo "  [type]             The type of disk (optional, valid options (standard, balanced, ssd), defaults to standard)"
  echo "  [instance]         The type of disk (optional, valid options (e2-micro, e2-small, e2-medium), defaults to e2-micro)"
}

main() {
  declare name="$1" size="${2:-25}" type="${3:-standard}" instance="${4:-"e2-micro"}"

  echo "Creating '${name}' with arguments size='${size}' type='${type}' instance='${instance}'"
  ...
  gcloud compute instances create "${name}" \
    ...
    --create-disk=auto-delete=yes,boot=yes,device-name=stream-template,image=projects/ubuntu-os-cloud/global/images/ubuntu-2004-focal-v20240830,mode=rw,size="${size}",type=projects/runloop-videos/zones/europe-west1-b/diskTypes/pd-"${type}" \
